...
export class InskeepCrawler {
    ...
    await retry({
      adapter: async () => {
        const res = await fetch('https://api.inskeep.cn/v2/app/api/user/init', {
          ...
          body,
        }).then(response => response.json() as unknown as Promise<BaseResponse<LoginData>>);
        if (res.errcode === 0) {
  ...
  private async fetch<T>(url: string, { method, params }: { method: 'GET' | 'POST'; params?: any }) {
    if (method === 'GET') {
      url = axios.getUri({
        baseURL: url,
        params: {
  ...
  async fetchUserRank(group_id: number, offset = 0) {
    ...
    const res = await this.fetch<UserList>('https://api.inskeep.cn/v2/app/api/owner/tag', {
      method: 'POST',
      params: {
  ...
  private async getUserInfo(starId: string, categoryId: number) {
    const user = await this.fetch<UserInfo>('https://api.inskeep.cn/v2/app/api/owner/user', {
      method: 'GET',
      params: {
        userId: starId,
      },
  ...
  private async searchUserInfoByName(fullName: string) {
    ...
    await retry({
      adapter: async () => {
        try {
          const res = await this.fetch<InskeepSearchData>('https://api.inskeep.cn/v2/app/api/owner/search', {
            method: 'POST',
            params: {
              keyword: fullName,
            },
  ...
  private async *getFirstPageByInn(options: {
    ...
    try {
      ...
      while (hasMore) {
        ...
        console.log(`获取到数据，cursor: ${cursor}，size: ${nextPageData.items.length}`);
  ...
  private async getNextPageData(userId: string, offset = 0) {
    try {
      ...
      await retry({
        ...
        adapter: async () => {
          try {
            const d = await this.fetch<InskeepwPostResponse>('https://api.inskeep.cn/v2/app/api/owner/media', {
              method: 'GET',
              params: {
                offset,
                userId,
  ...
  private extraPost(posts: InskeepPost[], startName?: string, fullName?: string) {
    ...
    const extraAttachment = (post: InskeepPost) => {
      return post.sources!.map(async item => ({
        ...
        thumbnail_url:
          item.type === 'video' ? post.mainImage.replace('http://', 'https://') : item.url.replace('http://', 'https://'),
  ...
  async run(options: { starId: string; categoryId?: number; fullName?: string; userName?: string; force?: boolean }) {
