...
export class UploadMedia {
  ...
  constructor() {
    ...
    this.multibar = new MultiBar({
      format: `{filename} | ${colors.cyan('{bar}')} | {percentage}% | {value}/{total} | ETA: {eta}s`,
      barCompleteChar: '\u2588',
      barIncompleteChar: '\u2591',
      hideCursor: true,
      ...
      await retry({
        async adapter() {
          try {
            const {
              data: { urls },
            } = await fetch('https://workers.19981105.xyz/common/overseas2cdn', {
              method: 'post',
  ...
  async uploadMulMeidaToCdn(posts: Post[]) {
    try {
      ...
      for (const post of posts) {
        ...
        complete++;
        console.log(`当前正在上传帖子(${post.insPostId})，总上传进度：${((complete * 100) / posts.length).toFixed(2)}%`);
        bar.increment();
      }
      bar.stop();
      return posts;
